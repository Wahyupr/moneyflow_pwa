...
function DebtCard({
  ...
  const progressPct = debt.total_amount_minor > 0
    ? Math.min(100, Math.round((debt.paid_amount_minor / debt.total_amount_minor) * 100))
    : 0;
  const dueDate = debt.next_due_date ? new Date(debt.next_due_date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) : null;
  const targetDate = debt.target_paid_off_date ? new Date(debt.target_paid_off_date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) : null;
    ...
    <article className="rounded-xl bg-surface p-4 shadow-card">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-muted">{debt.creditor_name} · {debt.category}</p>
        </div>
        <button type="button" onClick={onDelete} disabled={busy} className="text-muted transition hover:text-expense disabled:opacity-50" aria-label="Hapus">
          <Trash2 size={16} />
        </button>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <div>
          ...
          <p className="text-sm font-bold text-ink">{displayAmount(formatCurrency(debt.total_amount_minor, "IDR"))}</p>
        </div>
        <div>
          <p className="text-[11px] uppercase tracking-wider text-muted">Sisa Hutang</p>
          <p className="text-sm font-bold text-expense">{displayAmount(formatCurrency(debt.remaining_amount_minor, "IDR"))}</p>
        </div>
      </div>
      ...
      {debt.remaining_amount_minor > 0 ? (
        ...
        <p className="mt-3 rounded-lg bg-income/10 p-2 text-center text-xs font-semibold text-income">Lunas</p>
      )}
    </article>
  );
}
...
function DebtFormSheet({
  ...
  const effectiveCategory = useMemo(() => (isCustom ? customCategory.trim() : category), [isCustom, customCategory, category]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const monthlyMinor = monthlyInstallment.trim() === "" ? null : Math.round(Number(monthlyInstallment));
    ...
    if (monthlyMinor !== null && monthlyMinor <= 0) return setError("Cicilan bulanan harus lebih dari 0.");
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-3">
          <div className="space-y-3">
            ...
            <label className="block">
              ...
              <input className="mt-1 min-h-12 w-full rounded-lg border border-outline bg-surface px-3 focus:border-primary focus:outline-none" inputMode="numeric" value={totalAmount} onChange={(e) => setTotalAmount(e.target.value)} placeholder="350000000" />
            </label>
            ...
            <label className="block">
              <span className="text-sm font-semibold text-muted">Cicilan Bulanan (Rp) — opsional</span>
              <input className="mt-1 min-h-12 w-full rounded-lg border border-outline bg-surface px-3 focus:border-primary focus:outline-none" inputMode="numeric" value={monthlyInstallment} onChange={(e) => setMonthlyInstallment(e.target.value)} placeholder="2800000" />
            </label>
